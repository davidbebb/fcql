 ...
 * @param {boolean} validate - should only validate data and return error text if any, OTHERWISE throws exception.
 * @constructor
 */
function FluentCql(validate) {
        ...
        var clauses = _.map(
            ...
            function (keyValuePair) {
                ...
                if (_.isString(value)) {
                    ...
                    return [key, '=', _stringify(value)].join(' ');
                }
                    ...
                    return [key, 'IN', '(' + _.map(value, _stringify).join(', ') + ')'].join(' ');
                    ...
                    var clause = [key, conditionOps[operators[0][0]], _stringify(operators[0][1])];
                    if (operators.length === 2) {
                        ...
                        clause.push('AND');
                        clause = clause.concat([key, conditionOps[operators[1][0]], _stringify(operators[1][1])]);
                    }
     ...
     * @param {boolean} [durableWrites=] - durable writes boolean. Uses CQL default 'true' if not present.
     * See {@link http://cassandra.apache.org/doc/cql3/CQL.html#createKeyspaceStmt}
     * @returns {FluentCql}
     */
     ...
     * @param {object} [replication="{'class': 'SimpleStrategy', 'replication_factor': '3'}"] - replication strategy options.
     * See {@link http://cassandra.apache.org/doc/cql3/CQL.html#createKeyspaceStmt}
     * @param {boolean} [durableWrites=] - durable writes boolean. CQL defaults to 'true' if not present.
     * See {@link http://cassandra.apache.org/doc/cql3/CQL.html#createKeyspaceStmt}
     * @returns {FluentCql}
     */
    ...
    this._keyspaceOptions = function _keyspaceOptions(replication, durableWrites) {
        ...
        if (!replication || _.isEmpty(replication)) {
            replication = _defaultReplicationOptions();
        }
        else {
            if (!replication['class']) {
                replication['class'] = _defaultReplicationOptions()['class'];
            }
            if (_.isUndefined(replication.replication_factor)) {
                replication.replication_factor = _defaultReplicationOptions().replication_factor;
            }
            else {
        ...
        this.concat_(_stringify(replication));
    ...
    this.insertIntoValues = function insertIntoValues() {
        var args = [].slice.call(arguments);
        if (args.length <= 1) {
        ...
        return this
            ...
            .concat_('VALUES')
            .concat('(')
            .concat(_.map(values, _stringify).join(', '))
            .concat_(')');
    };
    ...
    this.values = function values(vals) {
        ...
        return this
            .concat_('VALUES')
            .concat('(')
            .concat(_.map(args, _stringify).join(', '))
            .concat_(')');
    };

    this.using = function using(vals) {
        var args = _.isArray(vals) ? vals : [].slice.call(arguments);
        if (args.length !== 1) {
            return this.setError('it must be single argument in USING');
        }

        this.concat_('USING');
        var options = args[0];
        if (_.isString(options)) {
            return this.concat_(options);
        }
        if (!_.isObject(options)) {
            return this.setError('argument must be object or string in USING');
        }
    ...
    this.delete = function deleteImpl(columns) {
        this.concat_('DELETE');

        var args = _.isArray(columns) ? columns : [].slice.call(arguments);
        if (args.length === 0) {
            return this;
        }
        ...
        return this.setError('wrong arguments in DELETE');
    };
}
